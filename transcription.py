...
def process_audio_and_send_email(url, api_key, api_server, email_sender, email_receiver):
    # 1. 获取网页源代码并解析
    ...
    if file_size > MAX_FILE_SIZE_MB * BYTES_PER_MB:
        ...
        # 转录整个音频文件
        response_json = send_audio_transcription_request(temp_file_path, api_key, api_server)
...
if __name__ == "__main__":
    ...
    # 调用函数处理音频并发送邮件
    result = process_audio_and_send_email(test_url, api_key, api_server, email_sender, email_receiver)
    print(result)
